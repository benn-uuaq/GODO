...
class VisionCore:
    ...
    def detect_circle_by_diff(self, color_img, depth_img, target_color="white"):
        ...
        # [윤곽선 추출용] Red 채널 배제하여 구리벽 윤곽 억제
        edge_img = small_color.copy()
        edge_img[:, :, 2] = 0  
        gray = cv2.cvtColor(edge_img, cv2.COLOR_BGR2GRAY)
        blurred = cv2.GaussianBlur(gray, (5, 5), 0)
        
        scale_ratio = small_color.shape[1] / (self.base_width * scale) 
        ...
        # ★ config에서 색상 임계값 불러오기 (기본값 20)
        color_threshold = self.v_config.get("COLOR_WEIGHT_THRESHOLD", 20)
        
        best_circle, best_score = None, -9999 
        ...
        for cnt in contours:
            ...
            if circularity > 0.6 and (math.pi * (min_r ** 2) * 0.4) <= area <= (math.pi * (max_r ** 2) * 1.6):
                (x, y), radius = cv2.minEnclosingCircle(cnt)
                
                mask_in = np.zeros(small_color.shape[:2], dtype=np.uint8)
                cv2.circle(mask_in, (int(x), int(y)), int(radius* 0.98), 255, -1) 
                cv2.circle(mask_in, (int(x), int(y)), int(radius * 0.85), 0, -1)   
                
                avg_bgr = cv2.mean(small_color, mask=mask_in)[:3]
                B, G, R = avg_bgr
                ...
                if target_color == "white":
                    ...
                    color_weight = 255 - ((B + G + R) / 3.0)
                elif target_color == "blue":
                    color_weight = B - ((G + R) / 2.0)
                elif target_color == "red":
                    color_weight = R - ((B + G) / 2.0)
                elif target_color == "purple":
                    color_weight = ((B + R) / 2.0) - G
                else:
                    color_weight = (B + G + R) / 3.0
                
                # ★ 하드코딩 숫자 대신 변수 사용
                if color_weight < color_threshold: continue

                dx, dy, dr = int(x / scale), int(y / scale), int(radius / scale)
                ...
                z_in = np.median(depth_img[(roi_mask_in == 255) & (depth_img > 0)])
                z_out = np.median(depth_img[(roi_mask_out == 255) & (depth_img > 0)])
                
                if z_in > 0 and z_out > 0:
                    depth_diff = abs(z_out - z_in) 
                    if 0.005 <= depth_diff <= 0.25:
                        score = color_weight * circularity * (depth_diff * 100)
                        if score > best_score:
                            best_score, best_circle = score, (dx, dy, dr)

        # =================================================================
        # 2. HoughCircles 기반 탐색 (안전망)
        # =================================================================
        if best_circle is None:
            circles = cv2.HoughCircles(blurred, cv2.HOUGH_GRADIENT, dp=1.5, minDist=300 * scale_ratio * scale, 
                                       param1=50, param2=45, 
            ...
            if circles is not None:
                ...
                for (x, y, r) in circles:
                    current_radius = float(r)
                    
                    mask_in = np.zeros(small_color.shape[:2], dtype=np.uint8)
                    cv2.circle(mask_in, (int(x), int(y)), int(current_radius* 0.98), 255, -1)
                    cv2.circle(mask_in, (int(x), int(y)), int(current_radius * 0.85), 0, -1)
                    ...
                    dx, dy, dr = int(x / scale), int(y / scale), int(current_radius / scale)
                    roi_mask_in = np.zeros(depth_img.shape, dtype=np.uint8)
                    cv2.circle(roi_mask_in, (dx, dy), int(dr * 0.7), 255, -1)
                    roi_mask_out = np.zeros(depth_img.shape, dtype=np.uint8)
                    cv2.circle(roi_mask_out, (dx, dy), int(dr * 1.3), 255, -1)
                    ...
                    if z_in > 0 and z_out > 0:
                        depth_diff = abs(z_out - z_in)
                        if 0.005 <= depth_diff <= 0.25:
                            score = color_weight * (depth_diff * 100)
                            if score > best_score:
                                best_score, best_circle = score, (dx, dy, dr)
    ...
    def _read_qr_robust(self, color_img):
        ...
        barcodes = zxingcpp.read_barcodes(binary)
        if barcodes: return barcodes
...
#             print("\n--- [B] 비커 외곽(원) 검출 모듈 테스트 ---")
#             t1 = time.time()
#             circle = vision.detect_circle_by_diff(color_img, depth_img)
#             t2 = time.time()
#             if circle:
#                 cx, cy, cr = circle
